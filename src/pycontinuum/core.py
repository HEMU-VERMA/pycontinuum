...
class Continuation[A, B]:
    ...
    def __init__(
        self,
        func: Callable[..., AsyncGenerator[Shift, A]],
        args: tuple,
        kwargs: dict,
        history: tuple,
    ...
    def __reduce__(self):
        return (
            Continuation,
            (self._func, self._args, self._kwargs, self._history, self._type_a, self._type_b),
        )
...
async def _replay_and_continue(func, args, kwargs, decisions):
    ...
    while True:
        ...
        if isinstance(event, Shift):
            idx += 1
            if idx > len(decisions):
                return await _handle_new_shift(
                    event, func, args, kwargs, tuple(decisions[: idx - 1])
                )
        else:
            return event
